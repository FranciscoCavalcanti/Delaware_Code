#################################################################################
#
# The purpose is to extract monthly data of moisture for every Brazilians AMC
#
...
#
# The outcome is .csv files by month and AMC moisture
#
#################################################################################
...
for (yr in 1969:2017){
  ...
  for (i in 27:39){
    ...
    setwd(in_dir)
    write.csv(df, 
              paste0(in_dir, "/amc_moisture_csv/", yr , month, "_amc_moisture.csv"), 
              row.names = TRUE,
              ) # overwrites
